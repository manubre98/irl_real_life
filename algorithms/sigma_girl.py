...
import scipy
from utils.utils import estimate_cov, estimate_distribution_params
...
def make_loss_function(mu, sigma, identity_mat, girl=False):
    P = np.dot(mu.T, mu)
    if girl:
        def compute_loss(w):
            w = np.reshape(w, (-1, 1))
            obj = np.dot(w.T, np.dot(P, w))
            return obj
    else:
        def compute_loss(w):
            w = np.reshape(w, (-1, 1))
...
def make_weights_assignment_function(mus, sigmas, ids, num_objectives, seed=None, num_iters=10, verbose=False,
    ...
    for agent in range(num_agents):
        loss_functions.append(make_loss_function(mus[agent], sigmas[agent], ids[agent], girl=girl))
    ...
    def calculate_weights(assignment):
        ...
        while i < num_iters:
            ...
            if res.success:
                ...
                i += 1
        if not success:
            return np.ones(num_objectives)/num_objectives, None
        evaluations = np.array(evaluations)
        min_index = np.argmin(evaluations[:, 1])
        x, y = evaluations[min_index, :]
        #pbar.close()
