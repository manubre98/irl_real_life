...
def compute_gradient(pi, x_dataset, y_dataset, r_dataset,
    ...
    states = []  # states[: int(states.shape[0] / ep_len) * ep_len]
    actions = []  # actions[: int(actions.shape[0] / ep_len) * ep_len]
    features = []  # features[: int(features.shape[0] / ep_len) * ep_len]
    base = 0
    base_year = 1945
    # ep_len = 365
    while base + episode_length <= x_dataset.shape[0]:
        st = x_dataset[base:base + episode_length]
        act = y_dataset[base:base + episode_length]
        ft = r_dataset[base:base + episode_length]
        ft = ft * np.tile(gamma, (r_dim, 1)).transpose()
        states.append(st)
        actions.append(act)
        features.append(ft)
        stepsize = 0
        for i in range(int(years_step)):
            if base_year % 4 == 0:
                stepsize += 366
            else:
                stepsize += 365
    ...
    for i in range(states.shape[0]):
        ...
        for j in range(states.shape[1]):
            ...
            if discrete:
                st = st.reshape(1, -1)
                action = [action]
            step_layers, p, logp, mean = pi.compute_gradients(st, action)
            step_gradients = []
            for layer in step_layers:
                step_gradients.append(layer.ravel())
            step_gradients = np.concatenate(step_gradients)
    ...
    #
    #     step_layers, p, logp, mean, std = pi.compute_gradients([x_dataset[step]], [y_dataset[step]])
    #     step_gradients = []
    #     for layer in step_layers:
    #         step_gradients.append(layer.ravel())
    #     step_gradients = np.concatenate(step_gradients)
