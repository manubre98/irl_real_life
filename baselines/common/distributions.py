...
class CategoricalPdType(PdType):
    ...
    def pdfromlatent(self, latent_vector, init_scale=1.0, init_bias=0.0,
                     trainable_variance=True, trainable_bias=True, init_logstd=0.,
                     clip=None, state_dependent_variance=False, beta=1.0):
        pdparam = fc(latent_vector, 'pi', self.ncat, init_scale=init_scale, init_bias=init_bias,
                     trainable_bias=trainable_bias)
        pdparam = beta * pdparam
        pdparam -= tf.expand_dims(tf.reduce_max(pdparam, axis=1), axis=1)
...
class DiagGaussianPdType(PdType):
    ...
    def pdfromlatent(self, latent_vector, init_scale=1.0,
        ...
        if state_dependent_variance:
            logstd = fc(latent_vector, 'pi/logstd', self.size, init_scale=init_scale, init_bias=init_logstd,
                       trainable_bias=True)
        else:
