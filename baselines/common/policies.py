...
class PolicyWithValue(object):
    ...
    def __init__(self, env, observations, latent, estimate_q=False, vf_latent=None,
                 sess=None, trainable_variance=True, init_logstd=0, init_bias=0., clip=None, **tensors):
        """
        Parameters:
        ----------
        env             RL environment
    ...
    def step(self, observation, stochastic=False, logits=False, **extra_feed):
        ...
        **extra_feed    additional data such as state or mask (names of the arguments should match the ones in constructor, see __init__)
        ...
        a, v, state, neglogp, std = self._evaluate([self.action, self.vf, self.state, self.neglogp, self.pd.std], observation, stochastic, **extra_feed)
        if state.size == 0:
            state = None
        return a, v, state, std
...
def build_policy(env, policy_network, value_network=None, normalize_observations=False, estimate_q=False,
                 trainable_variance=True, init_logstd=0, init_bias=0, clip=None, **policy_kwargs):
